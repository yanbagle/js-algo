...
WordDictionary.prototype.search = function (word) {
  ...
  const dfs = (root, j) => { // j is the index of current char in word
    let curr = root;
    
    for (let i = j; i < word.length; i++) {
      const c = word.charAt(i);
      if (c === '.') {
        for (const [,value] of Object.entries(curr.children)) { // iterate through curr's children because . can be anything
          // value are the characters 
          if (dfs(value, i + 1)) {
            return true;
          }
          return false;
