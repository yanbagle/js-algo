...
const insert = (intervals, newInterval) => {
  ...
  for (let i = 0; i < intervals; i++) {
    ...
    if (newInterval[1] < currInterval[0]) { // newInterval comes before current interval, e.g. NI = [1,2] CI = [3,5]
      res.push(newInterval);
      return [...res, ...intervals.slice(i)];
    } else if (newInterval[0] > currInterval[1]) { // newInterval comes after current interval, e.g. NI = [3,5] CI = [1,2]
      res.push(currInterval);
    } else { // overlapping
      const start = Math.min(newInterval[0], currInterval[0]);
      const end = Math.max(newInterval[1], currInterval[1]);
